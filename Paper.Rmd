...
After the ridge regression optimization model to create weights and apply the PerformanceScore, ExpectedScore and FinalScore to all the players in our dataset, a series of leaderboards filtered by position were returned.(Leaderboards will be in Appendix). When looking at Performance per Pick, the mean PerformanceScore/ExpectedScore is not a linear decline with variability as the last overall 60th pick has the highest ExpectedScore after the top 10 picks. Figure 1 shows the average PerformanceScore while taking both position and pick into account. The worst overall position and pick is the 32nd overall pick and the centers while the best is the first overall pick guards. The guards consistently have the biggest PerformanceScore, outperforming the 2 other positions while the forwards display the most steady and moderate values. The centers have the most variance in the graph, showing the extreme difficulty that is needed to draft a big in the modern NBA.

```{r echo=FALSE, message=FALSE}
library(ggplot2)
library(dplyr)
FinalDraftData <- read.csv("FinalDraftData.csv")
# Summarize average performance by position and pick
pos_pick_summary <- FinalDraftData %>%
  group_by(Position, Pick) %>%
  summarise(
    AvgPerformance = mean(PerformanceScore, na.rm = TRUE),
    .groups = "drop"
  )

# Plot with custom colors (no red)
ggplot(pos_pick_summary, aes(x = Pick, y = AvgPerformance, color = Position)) +
  geom_line(linewidth = 1.3) +
  geom_point(size = 2) +
  scale_color_manual(
    values = c("G" = "#1f77b4",   # blue
               "F" = "#2ca02c",   # green
               "C" = "#9467bd")   # purple
  ) +
  scale_x_continuous(breaks = seq(1, max(pos_pick_summary$Pick), by = 5)) +
  labs(
    title = "Average Player Performance by Draft Pick and Position",
    subtitle = "Colored lines and dots show average PerformanceScore per position",
    x = "Draft Pick",
...
After looking at the best and worst players per lottery pick, the natural question arises? When drafting in the lottery, what type of players should one expect per pick? Figures 3a, 3b and 3c examine the player closest to the average Performance Score for their pick among the top 15 picks in the draft. The blue line visualization demonstrates how average player performance declines with draft pick, but the curve is not perfectly linear in all 3 positions. Mid-lottery players over exceeded expectations at times, showing the natural variability of draft outcomes. (Looking at the graphs, mid-lottery players generally have higher averages smth abt this)
(need some help phrasing this part, will talk about in next session)
...
In this project, others may have doubts around the PerformanceScore formula which is why a webapp in which you can remake the formulas to fit what you think the ideal guard, forward and center should be scoring. The formulas in this project is purely a standard in which the average center, guard and forward can be assessed. Something else that could be used to find different weights is to use a different optimization model such as Lasso or Elastic Net regression. In attempting to create the best FinalScore formula, z-score was experimented with, before FinalScore was just a straight deviation from the mean. When using z-score, the results were very skewed and the leaderboards did not provide an accurate description of what the project was meant to do so the FinalScore formula was changed to PerformanceScore-ExpectedScore.  
