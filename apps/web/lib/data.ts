...
const resolveCsvPath = (filename: string) => {
  ...
  if (!found) {
    throw new Error(
      `Unable to locate ${filename}. Checked: ${candidates.join(', ')}. Ensure CSV files are deployed with the app.`
    );
  }
...
export const getDraftPlayers = (): DraftPlayer[] => {
  const csv = fs.readFileSync(resolveCsvPath('DraftData.csv'), 'utf8');
